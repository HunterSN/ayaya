...
def tcmspnumzong(herbs):
    for herb in herbs:
        ...
        browsertcmsp.quit()
        htmltcmsp = request.urlopen(tcmspurl).read().decode('utf-8')
        co = re.findall(r'"molecule_ID":".*?"',htmltcmsp)
        coc = str(len(co))
        print(herb + ':' + sherbname + ':' + coc)
        tcmspc = open('tcmspcn.txt','a+')
        for temptcmspc in co:
            tcmspc.write(herb + '$' + sherbname + '$' + 'tcmsp' + '$' + str(temptcmspc)+'\n')
            pass
        tcmspc.close()
...
def etcm(herb):
    ...
    for url in compoundsurl:
        writeurl = open(herb + 'compoundsurl etcm.csv','a+')
        writeurl.write(url.get_attribute('href') + '\n')
        writeurl.close()
        pass
    
    compoundurlectmfile = open(herb + 'compoundsurl etcm.csv')
    ...
    for compound in compoundurlectm:
        ...
        etcmpubchemnum = browseretcm.find_element_by_xpath('//*[@id="table"]/tbody/tr[29]/td[2]/div').text
        etcmcasnum = browseretcm.find_element_by_xpath('//*[@id="table"]/tbody/tr[28]/td[2]/div').text
        print(compoundname)
        print(etcmpubchemnum)
        print(etcmcasnum)
        compoundetcm = open(setcmherb + ' ' + 'etcmc.txt','a+')
        compoundetcm.write(herb + '$' + setcmherb + '$' + 'etcm' + '$' + compoundname + '$' + etcmpubchemnum +'$' + etcmcasnum +'\n')
        compoundetcm.close()
        ...
        for target in targetsname:
            ...
            targetetcm.write(herb + '$' + setcmherb + '$' + 'etcm' + '$' + compoundname + '$' + etcmpubchemnum +'$' + etcmcasnum + '$' + targetname + '\n')
            targetetcm.close()
            pass
        pass
    browseretcm.quit()
...
def etcmzong(herbs):
    for herb in herbs:
        ...
        for compound in compoundurlectm:
            ...
            compoundzongetcm.write(herb + '$' + setcmherb + '$' + 'etcm' + '$' + compoundname + '$' + etcmpubchemnum +'$' + etcmcasnum + '\n')
            ...
            for target in targetsname:
                ...
                targetezongtcm.write(herb + '$' + setcmherb + '$' + 'etcm' + '$' + compoundname + '$' + etcmpubchemnum +'$' + etcmcasnum + '$' + targetname + '\n')
...
shuzu = 'baiqian','yuxingcao','mahuang','pipaye','qianhu','lugen','chuanbeimu'

etcmshuzu ='BAI+QIAN','YU+XING+CAO','MA+HUANG','QIAN+HU','LU+GEN','CHUAN+BEI+MU'
...
#etcmlianjie('枇杷叶','http://www.nrc.ac.cn:9090/ETCM/index.php/Home/Index/yc_details.html?id=254')
